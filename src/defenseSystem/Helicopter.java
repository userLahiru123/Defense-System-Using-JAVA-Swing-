...
public class Helicopter extends JFrame implements Observer{
    ...
    public Helicopter(MainController obj){
        ...
        id = String.format("HL %0,4d",++INDEX);
        setTitle("Helicopter "+id);
        setSize(650,400);
        setResizable(false);
        setLayout(null);
        ...
        statusMeter.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                if(statusMeter.getValue() == 90 && increaseDamage){
                    sendDamageStatus(obj);
                }
                if(statusMeter.getValue() > 95){
                    btnShoot.setEnabled(false);
                    btnMissile.setEnabled(false);
                    btnRedar.setEnabled(false);
                }
                if(statusMeter.getValue() == 98){
                    increaseDeath = !increaseDeath;
                }
                if(statusMeter.getValue() == 98 && increaseDeath){
                    deathNotice(obj);
                }
            }
        });
    ...
    public void sendFuelStatus(MainController obj){
          obj.fuelInfoStatus("Helicopter "+id);
    }
    
    public void sendDamageStatus(MainController obj){
        obj.damageInfoStatus("Helicopter "+id);
    }
    
    public void deathNotice(MainController obj){
        obj.deathNotice("Helicopter "+id);
    }
    
    public void solderDeathNotice(MainController obj){
        obj.solderDeathNotice("Helicopter "+id);
    }
    
    public void sendMessageToMain(MainController obj){
        if(!txtMessage.getText().equals("")){
            obj.receiveMessageFromObserver(("Helicopter "+id), txtMessage.getText());
            if(lblDisplayMessage.getText().equals("")){
    ...
    public void sendPositionNotification(MainController obj){
        if(setPosition.isSelected()){
            if(obj.lblDisplayNotify.getText().equals("")){
                obj.lblDisplayNotify.setText("Helicopter "+id+" is positioned...");
            }else{
                obj.lblDisplayNotify.setText(obj.lblDisplayNotify.getText().trim()+ "\nHelicopter "+id+" is positioned...");
            }
        }else{
            obj.lblDisplayNotify.setText(obj.lblDisplayNotify.getText().trim()+ "\nHelicopter "+id+" is not positioned...");
        }
    }
}